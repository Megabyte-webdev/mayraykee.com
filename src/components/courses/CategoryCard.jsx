...
function CategoryCard({data}) {
    ...
    return ( 
    <li onClick={()=>navigate(`/dashboard/courses/preview/${data.id} ${data?.title}`, {state: {data: data, previousPath: location.pathname}})} className="px-[10px] min-w-40 min-h-20 group cursor-pointer hover:scale-105 duration-100 hover:shadow-md py-[5px] justify-between bg-white items-center rounded-[10px] flex gap-[2px]">
