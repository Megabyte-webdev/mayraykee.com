...
function RegistrationProgress({ state, dispatch }) {
      ...
      return (
        <li key={currentStep.title} className="w-full flex">
          <div className="flex flex-col items-center w-[25%]">
            ...
            >
              <img
                className="w-[20px] h-[20px] md:w-[25px] md:h-[25px] block"
                src={icon}
                alt={`${currentStep.title} icon`}
              />
            </div>
            {currentStep.title !== registration_steps_keys.welcome_video.title && (
              <hr className="h-[25px] border-dashed border w-0 hidden md:block" />
            )}
          </div>
          ...
          <div className="hidden md:block">
            ...
            <p className="font-medium text-small text-gray-400">
              {currentStep.desc}
            </p>
          </div>
        </li>
  ...
  return (
    <div className="w-full h-full bg-green flex items-center justify-center">
      <ul className="flex flex-col md:w-[60%]">{getSteps()}</ul>
    </div>
  );
}
